...
- [x] 45. Manual smoke test on `http://localhost:5173` with admin account: login redirect → dashboard KPIs → navigate each section → exercise filters → disable a test user → export CSV → verify audit_logs row
