...
|----|-----------|-------|-----------|
| DR-03 | `db/schema_postgresql.sql` declara 14 tablas; falta `session_ratings` (Evo 011) y columnas `sessions.hidden_at/hidden_reason` (Evo 012). Regenerar vía `pg_dump --schema-only` post `alembic upgrade head`. | Ag.03 | ✅ Resuelto 2026-05-24: agregadas `session_ratings`, `sessions.hidden_at/hidden_reason` + índice parcial + CHECK. Naming alineado con modelos (ck_*). |
...
| DR-11 | Constraint duplicada potencial: el modelo SQLAlchemy declara `audit_logs_actor_role_check` mientras la migración 008 crea `chk_audit_logs_actor_role`. Verificar con `\d audit_logs` post-deploy; alinear naming. | Ag.03 | ✅ Resuelto 2026-05-24: modelo `audit_log.py:16` ahora declara `chk_audit_logs_actor_role` (alineado con migración 008 y DDL). |
...
| DR-14 | `empathy_rating.py:34` (modelo SQLAlchemy) tiene un comentario "[Evolución 008]" cuando el cambio realmente vive en migración `009_greeting_unique_empathy_updated`. Bug menor de doc-en-código. | Ag.03 | ✅ Resuelto 2026-05-24: comentario corregido a `[Migración Alembic 009_greeting_unique_empathy_updated]`. |
| DR-16 | Migraciones 011 + 012 NO son idempotentes (`op.create_table` y `op.add_column` sin `IF NOT EXISTS`). Si una BD se bootstrapean desde `db/schema_postgresql.sql` (que ya contiene esas tablas/columnas tras 2026-05-24) y luego se corre `alembic upgrade head`, las migraciones fallan con "relation/column already exists". La migración 008 sí es idempotente con `DO $$ IF NOT EXISTS`. **Workaround**: tras bootstrap desde DDL, usar `alembic stamp head` (marca migrations como aplicadas sin correrlas). Long-fix: agregar guards a 011 y 012. | Ag.03 | Media (antes del piloto) |
| DR-17 | `session_ratings.updated_at` (mig 011) no se auto-actualiza al UPDATE. Tiene `server_default=now()` pero sin trigger `BEFORE UPDATE` ni assign explícito en el repo. Si el estudiante re-califica, `updated_at` queda en el insert original — métrica inservible para "cuándo editó la última vez". Fix: agregar trigger SQL o setear explícitamente `updated_at=func.now()` en el upsert del repo. | Ag.03 + Ag.13 | Baja |
| DR-18 | No hay `MetaData(naming_convention=...)` configurada en `backend/app/models/base.py` ni `compare_type=True` en `alembic/env.py`. Causa raíz histórica de DR-11/DR-12: cada CHECK/FK/IX/PK/UQ se autonombra según quien lo crea (SQLAlchemy autogenerate vs Alembic manual vs Postgres anónimo). Cada migración futura puede reintroducir drift. Fix: configurar convención `{ck_, fk_, ix_, pk_, uq_}` en `MetaData` y una migración de re-naming masivo. | Ag.03 | Media |
